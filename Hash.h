...
public:
	...
	// 2 if rehash fails.
	// 3 if the table is about to overflow
...
private:
	...
	//				and the other field are meaningless.
	...
	public:
		...
		hashItem();
		hashItem(const std::string &k, void  *p = NULL);
	};

	int capacity; // The current capacity of the hash table.
	int filled; // Number of occupied items in the hash table.
